...
expected<unique_ptr<opentracing::SpanContext>> Tracer::Extract(const TextMapReader& reader) const {
  return ExtractImpl(reader);
}

expected<unique_ptr<opentracing::SpanContext>> Tracer::Extract(const HTTPHeadersReader& reader) const {
  return ExtractImpl(reader);
}
